...
(function (window) {
  ...
  function stpdVideo(stpdVideo) {
    ...
    stpdVideo.run = function ({ config, containerId }) {
      ...
      if (debug) {
        console.log(containerId + ': Window inner width: ' + top.window.innerWidth);
      }
      let player;
      let options;
      ...
      function createVideoElement() {
        if (debug) {
          console.log(containerId + ': Created');
        }
        if (videoContainer) {
          ...
          if (thumbnailSrc) {
            htmlContent += ` poster="${thumbnailSrc}"`;
          }
          if (thumbnailTitle) {
            htmlContent += ` title="${thumbnailTitle}"`;
          }
          htmlContent += `>
          ...
          pauseButton = document.querySelector(
            '#stpd_video_' + iterationId + ' .vjs-play-control .vjs-icon-placeholder'
          );
          ...
          scrollerThumbnails = document.querySelectorAll(
            '#scroller_' + iterationId + ' > .stpd-thumbnail-container'
          );
          ...
          if (!adUnit.includes('plcmt=')) {
            adUnit += '&plcmt=1';
          }
          if (!adUnit.includes('vid_d=')) {
            adUnit += '&vid_d=10000';
          }

          if (debug) {
            console.log(containerId + ': AdUnit url: ' + adUnit);
          }
      ...
      function initializeAds() {
        ...
        function onAdEvent(event) {
          ...
          if (event.type == google.ima.AdEvent.Type.CONTENT_PAUSE_REQUESTED) {
            ...
            if (debug) {
              console.log(containerId + ': Ad event:', event.type);
            }
          }
        }
      ...
      function pausePlayer() {
        ...
        if (
          ...
          (isElementOutOfView(videoContainer) &&
            (miniPlayerMobile.length <= 0 || miniPlayerClosed) &&
            isMobile &&
            !playerManuallyPaused)
        ) {
          if (!playerPaused) {
      ...
      function trackPlaylist(trigger, index) {
        const currentVideo = scroller.querySelectorAll('.stpd-thumbnail-container')[
          playlistProgressIndex
        ];
        ...
        const nextVideo = scroller.querySelectorAll('.stpd-thumbnail-container')[
          playlistProgressIndex
        ];
        nextVideo.classList.add('active');
        if (debug) {
          console.log(containerId + ': Current playlist index: ' + playlistProgressIndex);
        }
      }
      ...
      function videoEndedCheck(p) {
        p.on('ended', function () {
          videoEnded = true;
          if (debug) {
            console.log(containerId + ': Video has ended: ' + videoEnded);
          }
          autoplayPlaylist(p);
        });
      }
      ...
      function autoplayPlaylist(player) {
        if (!adBreakActive && videoEnded) {
          trackPlaylist(1);
          const nextVideoSrc =
            scrollerThumbnails[playlistProgressIndex].getAttribute('data-video-src');
          let nextVideoThumbnail = scrollerThumbnails[playlistProgressIndex]
            .querySelector('.stpd-thumbnail-img')
            .getAttribute('src');
          let nextVideoTitle =
            scrollerThumbnails[playlistProgressIndex].querySelector('.video-title').innerHTML;
          ...
          scrollerThumbnails.forEach(function (thumbnail, index) {
            thumbnail.addEventListener('click', function () {
              if (!adBreakActive) {
                ...
                let nextVideoThumbnail = scrollerThumbnails[index]
                  .querySelector('.stpd-thumbnail-img')
                  .getAttribute('src');
                let nextVideoTitle =
                  scrollerThumbnails[index].querySelector('.video-title').innerHTML;
                videoElement.setAttribute('poster', nextVideoThumbnail);
                videoElement.setAttribute('title', nextVideoTitle);
